...
% "Installing via Quarto extension" in the main README), where this
% file loads BEFORE brand.tex (the extension mechanism forces that
% order, the opposite of what a plain include-in-header list would
% do) — so \color{giInk} would otherwise fire before giInk is even
% defined. Confirmed via an actual render, not assumed.
