...
14. [Cover page](#14-cover-page)
15. [Editing brand colors and fonts](#15-editing-brand-colors-and-fonts)
16. [Writing tables](#16-writing-tables)
17. [Figures](#17-figures)
18. [Diagrams (Mermaid, Graphviz, and other tools)](#18-diagrams-mermaid-graphviz-and-other-tools)
19. [Landscape (wide) pages](#19-landscape-wide-pages)
20. [Footnotes](#20-footnotes)
21. [Citations and bibliography](#21-citations-and-bibliography)
22. [Page breaks](#22-page-breaks)
23. [Adding back-matter pages](#23-adding-back-matter-pages)
24. [Common mistakes and what actually goes wrong](#24-common-mistakes-and-what-actually-goes-wrong)
...
`_extensions/colophon/` (the engine — see README's "Getting a
project" section for why it lives there), and so on. Open that
**folder** in VS Code (File → Open Folder — not File → Open, which
opens a single file).

If you already have a Quarto book project of your own and just want
this toolkit's machinery rather than its example content, `quarto add
belpub/colophon-quarto` pulls in the extension alone — see README for
the full detail on that path.
...
[§24](#24-common-mistakes-and-what-actually-goes-wrong) for the
...
| `example.qmd`, `landscape-chapter-example.qmd`, `appendix-example.qmd` | Working, executing demonstrations of every distinctive pattern this toolkit adds — captioned tables/figures, a landscape-only chapter, a real lettered appendix. Safe to delete once you've started your own content. |
...
| `brand.tex` | Colors, fonts, and the book title used in the running header. The one file you edit to make this toolkit look like *your* brand — see [§15](#15-editing-brand-colors-and-fonts). |
| `cover-page.tex`, `cover-placeholder.png` | An optional full-bleed cover page, off by default — see [§14](#14-cover-page). |
| `_extensions/colophon/preamble.tex` | The structural engine — page layout, table mechanics, header/footer behavior, front/back-matter page numbering. Brand-agnostic; shouldn't need to change. Packaged as a Quarto project-type extension (see README's "Getting a project" section) — that's why it lives here rather than at the project root alongside `brand.tex`. |
...
exactly as documented in [§19](#19-landscape-wide-pages). No other
...
Off by default. To add a full-bleed cover image before the title
page:

1. Add `cover-page.tex` to `include-before-body` in `_quarto.yml`,
   right before `title-page.tex`:
   ```yaml
   include-before-body:
     - cover-page.tex
     - title-page.tex
   ```
2. Set `\giCoverPageImage` in `title-page-content.tex` to your own
   image file. It defaults to `cover-placeholder.png`, sized
   correctly (1240×1754px) as a working example to match.

The image stretches to fill the page exactly rather than getting
cropped, so it needs to already be sized to A4's proportions
(roughly 0.707:1, width to height) to avoid looking distorted.

This produces true full-bleed *within the page's own trim size* —
the image reaches every edge of the A4 page, with none of the rest
of the book's normal margin. It is not press-ready bleed for a
physically printed, trimmed cover, which usually needs to extend a
few millimeters past the trim edge and is normally supplied as a
separate file built to the printer's own template. See README's
"Cover page" section for the full distinction if this book is ever
headed to a professional print shop.

## 15. Editing brand colors and fonts
...
LaTeX equivalent of a figure's `fig-alt`. See [§17](#17-figures)
...
## 18. Diagrams (Mermaid, Graphviz, and other tools)

Don't use Quarto's native `` ```{mermaid} ``/`` ```{dot} `` code
blocks for PDF output — both require a headless Chrome install that
turned out to be genuinely fragile (it failed outright in one real
environment, due to network restrictions) and matches a long trail
of upstream bugs specific to PDF rendering: cropped diagrams,
indefinite hangs, failures once a document has more than one diagram
in it. HTML output doesn't have these problems; PDF does, and PDF is
...
## 24. Common mistakes and what actually goes wrong
...
| Label a raw-LaTeX table `\label{tab:x}` instead of `\label{tbl-x}` | Render succeeds, but you get a "non-tbl label" warning and can't `@tbl-x`-reference it elsewhere | Quarto's crossref system specifically looks for the `tbl-` prefix. Harmless to leave, free to fix — see [§16](#16-writing-tables). |
...
| Give a chapter only landscape content (no portrait text first) and use a normal Markdown heading anyway | Wastes a whole page: the title sits alone on an otherwise-blank portrait page before the landscape content starts on the next page | Use `\giLandscapeChapterOpener{Title}` + `\giLandscapeChapterHeader` instead — see [§19](#19-landscape-wide-pages) — which shows the title horizontally as part of the landscape page's own header instead of a separate page. |
